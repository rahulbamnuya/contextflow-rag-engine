"""
Graph builder module for the ContextFlow RAG system.
"""
...
def hallucination_checker(state: State):
    """
    Verify if the generated answer is faithful to the retrieved context.
    """
    final_answer = state["messages"][-1].content
    
    # Context is the retrieved chunk at the second-to-last index in the messages list
    ...
    chain = verify_prompt | llm_with_verification
    try:
        result = chain.invoke({
            "question": question,
            "context": context,
        ...
        print(f"Hallucination check results: faithful={result.faithful}, reason={result.explanation}")
        
        if not result.faithful:
            warning_msg = f"{final_answer}\n\n⚠️ *Warning: Some parts of this response could not be verified by the retrieved source documents and may be hallucinated.*"
            return {"messages": [{"role": "assistant", "content": warning_msg}]}
